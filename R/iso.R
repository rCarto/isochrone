...
#' detailed isochrones.
#' @param method select the iso creation method. It can be "osrm_pkg" or "smooth". 
#' "osrm_pkg" uses the same algorithm than the osrm package. "smooth" uses a 
#' moving window with a gaussian blur. "smooth" is usefull to remove small 
#' patches of area hard to reach.  
#' @param k size (sigma) of the gaussian moving window.
...
#' st_as_sfc st_geometry st_sf
#' @importFrom mapiso mapiso
#' @importFrom terra rast res focalMat focal
#' @export
#' @examples
#' \dontrun{
#' library(sf)
#' # Get isochones with lon/lat coordinates
#' iso <- isochrone(loc = c(13.43, 52.47), breaks = seq(0, 12, 2))
#' # Map
#' plot(iso["isomax"], breaks = sort(unique(c(iso$isomin, iso$isomax))))
#' }
isochrone <- function(loc, breaks = seq(from = 0, to = 60, length.out = 7),
                          exclude, res = 30, method = "osrm_pkg", k,
  ...
  if (min(sgrid$measure, na.rm = TRUE) >= tmax + 1) {
    warning(
      paste0(
  ...
  if (method == "smooth") {
    r <- rast(sgrid[, c("COORDX", "COORDY", "measure"), drop = TRUE], 
              crs = "epsg:3857")
    ...
    mat <- focalMat(x = r, d = k, type = "Gauss")
    ...
    sgrid <- focal(x = r, w = mat, fun = mean, na.rm = TRUE)
